...
class Brain:
    def __init__(self, dim_embedding: int = 64, input_neurons: int = 30, output_neurons: int = 30,
        ...
        # Инициализация нейронов (оставляем как было)
    ...
    # ---------- Вспомогательные методы (без изменений, кроме новых) ----------
    ...
    # ---------- Новый метод: извлечение понятий ----------
    def extract_concepts(self, text: str) -> List[str]:
        """Возвращает список изученных понятий, присутствующих в тексте (по точному вхождению)."""
        text_norm = text.lower().strip()
        found = []
        # Сортируем по длине (самые длинные сначала) для избежания вложенных совпадений
        for concept in sorted(self.concept_index.keys(), key=len, reverse=True):
            if concept in text_norm:
                found.append(concept)
                # Удаляем найденное, чтобы не дублировать
                text_norm = text_norm.replace(concept, '')
        return found

    # ---------- Генерация одной цепочки (выделено) ----------
    def _generate_chain_from_node(self, start_id: int, max_hops: int = 6,
                                   min_weight: float = 0.02, temperature: float = 0.0) -> List[str]:
        """Строит цепочку понятий, начиная с заданного нейрона."""
    ...
    # ---------- Обновлённый метод generate_answer ----------
    def generate_answer(self, input_text: str, max_hops: int = 6,
                        min_weight: float = 0.02, temperature: float = 0.7,
                        mode: str = 'chain') -> Dict[str, Any]:
        """
        Генерирует ответ, используя все понятия, найденные в вопросе.
        При mode='random' возвращает случайное понятие.
        """
        if mode == 'random':
        ...
        # 1. Извлекаем все понятия из запроса
        ...
        # 2. Если понятий не найдено, пробуем через infer (fallback)
        if not start_ids:
            sig = self.text_to_signal(input_text)
            top = self.infer(sig, input_text=input_text, top_k=3)
            for label, _ in top:
                nid = self.concept_index.get(label.strip().lower())
                if nid is not None and nid in self.neurons:
                    start_ids.append(nid)
            if not start_ids:
                return {
        ...
        # 3. Для каждого стартового узла строим цепочку
        all_chains = []
        for sid in start_ids:
            chain = self._generate_chain_from_node(
                sid, max_hops=max_hops, min_weight=min_weight, temperature=temperature
            )
            if chain and len(chain) > 1:
                all_chains.append(chain)
            else:
                # если цепочка короткая, добавляем начальное понятие как отдельный элемент
                all_chains.append([self.neurons[sid].label or "?"])

        # 4. Объединяем все цепочки в один текст
        if not all_chains:
            return {"chain": [], "text": "Не удалось сгенерировать ответ.", "known": False, "fallback": True}

        # Конкатенируем цепочки через " ; "
        combined_text = " ; ".join([" → ".join(chain) for chain in all_chains])

        # Собираем все уникальные метки в один список (для обратной связи)
        flat_chain = []
        for chain in all_chains:
            flat_chain.extend(chain)
...
def interactive_with_teacher(brain: Brain):
    ...
    try:
        while True:
            ...
            # Теперь температура выбирается в диапазоне 0.7–1.5
            temp = random.uniform(0.7, 1.5)
